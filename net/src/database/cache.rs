...
#[cfg(feature = "pool")]
impl From<bb8::RunError<rusqlite::Error>> for SQLiteCacheError {
    fn from(error: bb8::RunError<rusqlite::Error>) -> Self {
        match error {
            bb8::RunError::User(sql_error) => sql_error.into(),
            bb8::RunError::TimedOut => SQLiteCacheError::TimedOut,
        }
    }
}
...
impl SQLiteCache {
    ...
    pub async fn get(
        ...
        query: Query,
        now: DateTime<Utc>,
    ) -> Result<Option<QueryLookup>, SQLiteCacheError> {
        let mut connection = self.manager.get().await?;
        crate::block_in_place(|| {
            let tx = connection.transaction()?;
            let qx = QueryPersistence::new(&tx);
    ...
    ///
    /// Returns a `CacheError` if the database operation fails.
    pub async fn cleanup(&self, now: DateTime<Utc>) -> Result<(), SQLiteCacheError> {
        let mut connection = self.manager.get().await?;
        crate::block_in_place(|| {
            let tx = connection.transaction()?;
            let qx = QueryPersistence::new(&tx);
...
mod tests {
    ...
    fn test_cache_error_display() {
        let timeout_error = SQLiteCacheError::TimedOut;
        assert_eq!(
            timeout_error.to_string(),
            "Timed Out waiting for SQLite connection"
        );

        let db_error = SQLiteCacheError::Database(rusqlite::Error::InvalidColumnIndex(5));
