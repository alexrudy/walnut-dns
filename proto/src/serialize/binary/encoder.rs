...
mod private {
    ...
    impl<'a> MaximalBuf<'a> {
        ...
        pub(super) fn write(&mut self, offset: usize, data: &[u8]) -> Result<(), ProtocolError> {
            debug_assert!(offset <= self.buffer.len());
            if offset + data.len() > self.max_size {
                return Err(ProtocolError::MaxBufferSizeExceeded(self.max_size));
            }
        ...
        pub(super) fn reserve(&mut self, offset: usize, len: usize) -> Result<(), ProtocolError> {
            let end = offset + len;
        ...
        pub(super) fn bytes(self) -> &'a Vec<u8> {
            self.buffer
        }
    }
}
...
impl<'a> BinEncoder<'a> {
    ...
    // fn enforce_size(&mut self, additional: usize) -> Result<(), ProtocolError> {
    //     if (self.buffer.len() + additional) > self.max_size {
    //         Err(ProtocolErrorKind::MaxBufferSizeExceeded(self.max_size).into())
    //     } else {
    ...
    pub fn emit(&mut self, b: u8) -> Result<(), ProtocolError> {
    ...
    pub fn emit_character_data<S: AsRef<[u8]>>(
        ...
        if char_bytes.len() > 255 {
            return Err(ProtocolError::CharacterDataTooLong {
                max: 255,
                len: char_bytes.len(),
            });
        }
    ...
    /// Emit one byte into the buffer
    pub fn emit_u8(&mut self, data: u8) -> Result<(), ProtocolError> {
        self.emit(data)
    }

    /// Writes a u16 in network byte order to the buffer
    pub fn emit_u16(&mut self, data: u16) -> Result<(), ProtocolError> {
        self.write_slice(&data.to_be_bytes())
    }

    /// Writes an i32 in network byte order to the buffer
    pub fn emit_i32(&mut self, data: i32) -> Result<(), ProtocolError> {
        self.write_slice(&data.to_be_bytes())
    }

    /// Writes an u32 in network byte order to the buffer
    pub fn emit_u32(&mut self, data: u32) -> Result<(), ProtocolError> {
        self.write_slice(&data.to_be_bytes())
    }

    fn write_slice(&mut self, data: &[u8]) -> Result<(), ProtocolError> {
    ...
    pub fn emit_vec(&mut self, data: &[u8]) -> Result<(), ProtocolError> {
        self.write_slice(data)
    }
    ...
    /// Emits all the elements of an Iterator to the encoder
    pub fn emit_all_refs<'r, 'e, I, E>(&mut self, iter: I) -> Result<u16, ProtocolError>
    where
        'e: 'r,
            ...
            if let Err(e) = i.emit(self) {
                return Err(match &e {
                    ProtocolError::MaxBufferSizeExceeded(_) => {
                        ...
                        ProtocolError::NotAllRecordsWritten { count }
                    }
                    _ => e,
                });
            }
    ...
    pub fn place<T: EncodedSize>(&mut self) -> Result<Place<T>, ProtocolError> {
...
impl<T: EncodedSize> Place<T> {
    ...
    pub fn replace(self, encoder: &mut BinEncoder<'_>, data: T) -> Result<(), ProtocolError> {
