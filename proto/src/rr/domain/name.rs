...
use crate::serialize::binary::encoder::NameEncoding;
use crate::serialize::binary::{
    BinDecodable, BinDecoder, BinEncodable, BinEncoder, DecodeError, Restrict,
};
use crate::serialize::text::error::ParseError;
...
impl Name {
    ...
    ///
    /// ```
    /// use walnut_proto::rr::domain::Name;
    ///
    /// let root = Name::root();
    ...
    /// ```
    pub fn append_label<L: IntoLabel>(mut self, label: L) -> Result<Self, ProtocolError> {
    ...
    /// assert_eq!(name, Name::from_str("www.example.com").unwrap());
    /// ```
    pub fn prepend_label<L: IntoLabel>(&self, label: L) -> Result<Self, ProtocolError> {
    ...
    /// assert!(root.is_root());
    /// ```
    pub fn from_labels<I, L>(labels: I) -> Result<Self, ProtocolError>
    where
    ...
    /// assert!(name.is_fqdn());
    /// ```
    pub fn append_name(mut self, other: &Self) -> Result<Self, ProtocolError> {
    ...
    /// assert!(name.is_fqdn())
    /// ```
    pub fn append_domain(self, domain: &Self) -> Result<Self, ProtocolError> {
    ...
    /// use std::str::FromStr;
    ///
    /// use walnut_proto::rr::domain::{Label, Name};
    ///
    ...
    /// ```
    pub fn parse(local: &str, origin: Option<&Self>) -> Result<Self, ProtocolError> {
    ...
    /// assert_eq!(bytes_name, name);
    /// ```
    pub fn from_ascii<S: AsRef<str>>(name: S) -> Result<Self, ProtocolError> {
    ...
    /// ```
    pub fn from_utf8<S: AsRef<str>>(name: S) -> Result<Self, ProtocolError> {
    ...
    /// ```
    pub fn from_str_relaxed<S: AsRef<str>>(name: S) -> Result<Self, ProtocolError> {
            ...
            match state {
                    ...
                    if ch.is_numeric() {
                        state =
                            ParseState::Escape2(ch.to_digit(8).ok_or_else(|| {
                                ProtocolError::from(format!("illegal char: {ch}"))
                            })?);
                    } else {
                ...
                ParseState::Escape2(i) => {
                    if ch.is_numeric() {
                        ...
                        return Err(ProtocolError::from(format!("unrecognized char: {ch}")));
                    }
                }
                ParseState::Escape3(i, ii) => {
                    if ch.is_numeric() {
                        ...
                        let new: char = char::from_u32(val)
                            .ok_or_else(|| ProtocolError::from(format!("illegal char: {ch}")))?;
                        label.push(new);
                        state = ParseState::Label;
                    } else {
    ...
    pub fn parse_arpa_name(&self) -> Result<IpNet, ProtocolError> {
        if !self.is_fqdn() {
        ...
        let first = iter
            .next()
            .ok_or_else(|| ProtocolError::from("not an arpa address"))?;
        if !"arpa".eq_ignore_ascii_case(core::str::from_utf8(first)?) {
            return Err("not an arpa address".into());
        }
        let second = iter
            .next()
            .ok_or_else(|| ProtocolError::from("invalid arpa address"))?;
...
trait LabelEnc {
    fn to_label(name: &str) -> Result<Label, ProtocolError>;
...
pub trait IntoName: Sized {
    ...
    fn into_name(self) -> Result<Name, ProtocolError>;
...
mod tests {
    ...
    fn test_excessive_encoding_len() {
        ...
        match result.unwrap_err() {
            ProtocolError::MaxBufferSizeExceeded(_) => (),
            _ => panic!(),
        }
    }
    ...
    fn test_name_too_long_with_prepend() {
        ...
        match error {
            ProtocolError::Decode(DecodeError::DomainNameTooLong(_)) => (),
    ...
    fn test_length_limits() {
        ...
        assert!(matches!(
            long_label_error,
            ProtocolError::Decode(DecodeError::LabelBytesTooLong(64))
        ));
        let long_name_error =
            Name::parse(&format!("a.{}", &expected_name_str[1..]), None).unwrap_err();
        assert!(matches!(
            long_name_error,
            ProtocolError::Decode(DecodeError::DomainNameTooLong(256))
        ))
    }
