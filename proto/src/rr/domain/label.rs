...
impl Label {
    ...
    pub fn from_raw_bytes(bytes: &[u8]) -> Result<Self, ProtocolError> {
    ...
    pub fn from_utf8(s: &str) -> Result<Self, ProtocolError> {
...
pub trait IntoLabel: Sized {
    ...
    fn into_label(self) -> Result<Label, ProtocolError>;
}
...
mod tests {
    ...
    fn assert_panic_label_too_long(error: Result<Label, ProtocolError>, len: usize) {
        // poor man debug since ProtoResult don't implement Partial Eq due to ssl errors.
        eprintln!("{error:?}");
        assert!(error.is_err());
        match error.unwrap_err() {
            ProtocolError::Decode(DecodeError::LabelBytesTooLong(n)) if n == len => (),
            ProtocolError::Decode(DecodeError::LabelBytesTooLong(e)) => {
                panic!(
