...
impl TSigResponseContext {
    ...
    #[cfg(feature = "__dnssec")]
    pub fn sign(self, response: &[u8]) -> Result<Box<Record<TSIG>>, ProtocolError> {
        match self.kind {
            TsigResponseKind::Signed {
                ...
                let resp_tsig = stub_tsig.set_mac(
                    signer
                        ...
                        .map_err(|e| ProtocolError::from(e.to_string()))?,
                );
...
impl TSigVerifier {
    ...
    /// # Returns
    ///
    /// Returns the verified `DnsResponse` on success, or a `ProtocolError` if
    /// verification fails.
    pub fn verify(&mut self, response_bytes: &[u8]) -> Result<Message, ProtocolError> {
        ...
        if rt >= self.remote_time && range.contains(&self.request_time) {
            ...
            Err(ProtocolError::from(
                "tsig validation error: outdated response",
            ))
        }
    }
}
...
mod tests {
    ...
    fn test_sign_and_verify_message_tsig_reject_keyname() {
        ...
        signature.name_labels = other_name;
