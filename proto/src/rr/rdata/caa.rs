...
use crate::{
    ...
    serialize::{
        binary::{encoder::RDataEncoding, restrict::RestrictedMath, *},
        text::ParseError,
    },
};
...
impl CAA {
    ...
    pub fn set_issuer_value(
        ...
        if !self.tag.eq_ignore_ascii_case("issue") && !self.tag.eq_ignore_ascii_case("issuewild") {
            return Err((
                RecordType::CAA,
                "CAA property tag is not 'issue' or 'issuewild'",
            )
                .into());
        }
    ...
    pub fn set_iodef_value(&mut self, url: &Url) -> Result<(), ProtocolError> {
        if !self.tag.eq_ignore_ascii_case("iodef") {
            return Err((RecordType::CAA, "CAA property tag is not 'iodef'").into());
        }
    ...
    pub fn value_as_issue(&self) -> Result<(Option<Name>, Vec<KeyValue>), ProtocolError> {
        if !self.tag.eq_ignore_ascii_case("issue") && !self.tag.eq_ignore_ascii_case("issuewild") {
            return Err((
                RecordType::CAA,
                "CAA property tag is not 'issue' or 'issuewild'",
    ...
    pub fn value_as_iodef(&self) -> Result<Url, ProtocolError> {
        if !self.tag.eq_ignore_ascii_case("iodef") {
            return Err((RecordType::CAA, "CAA property tag is not 'iodef'").into());
        }
...
/// URL syntax).
pub fn read_issuer(bytes: &[u8]) -> Result<(Option<Name>, Vec<KeyValue>), ProtocolError> {
    ...
    let key_values = match state {
        ...
        ParseNameKeyPairState::Key { key, .. } => {
            return Err((RecordType::CAA, format!("key missing value: {key}")).into());
        }
    };
...
/// ```
pub fn read_iodef(url: &[u8]) -> Result<Url, ProtocolError> {
...
fn emit_tag(buf: &mut [u8], tag: &str) -> Result<u8, ProtocolError> {
    let len = tag.len();
    if len > u8::MAX as usize {
        return Err((RecordType::CAA, format!("CAA property too long: {len}")).into());
    }
    if buf.len() < len {
        return Err((
            RecordType::CAA,
...
impl BinEncodable for CAA {
    fn emit(&self, encoder: &mut BinEncoder<'_>) -> Result<(), ProtocolError> {
        ...
        encoder.emit_vec(self.value.as_slice())?;
