...
impl NSIDPayload {
    ...
    ///
    /// A `ProtocolError` is returned if the provided data is too large to be
    /// expressed as an EDNS option value.
    pub fn new(data: impl Into<Vec<u8>>) -> Result<Self, ProtocolError> {
        let data = data.into();
        if data.len() > u16::MAX as usize {
            return Err(ProtocolError::from("NSID EDNS payload too large"));
        }
        Ok(Self(data))
    }
}
