...
impl FromStr for SvcParamKey {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        fn parse_unknown_key(key: &str) -> Result<SvcParamKey, ProtocolError> {
            let key_value = key.strip_prefix("key").ok_or_else(|| {
                ProtocolError::Msg(format!("bad formatted key ({key}), expected key1234"))
            })?;
...
impl BinEncodable for SvcParamValue {
    ...
    fn emit(&self, encoder: &mut BinEncoder<'_>) -> Result<(), ProtocolError> {
        ...
        let len = u16::try_from(encoder.len_since_place(&place))
            .map_err(|_| ProtocolError::from("Total length of SvcParamValue exceeds u16::MAX"))?;
...
impl BinEncodable for Mandatory {
    ...
    fn emit(&self, encoder: &mut BinEncoder<'_>) -> Result<(), ProtocolError> {
        if self.0.is_empty() {
            return Err(ProtocolError::from("Alpn expects at least one value"));
        }
    ...
    fn emit(&self, encoder: &mut BinEncoder<'_>) -> Result<(), ProtocolError> {
        ...
        for (key, param) in self.svc_params.iter() {
            if let Some(last_key) = last_key {
                if key <= &last_key {
                    return Err(ProtocolError::from("SvcParams out of order"));
                }
            }
...
mod tests {
    ...
    use crate::{rr::rdata::HTTPS, serialize::text::Parser};
