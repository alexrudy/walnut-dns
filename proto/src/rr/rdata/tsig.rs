...
impl BinEncodable for TSIG {
    ...
    fn emit(&self, encoder: &mut BinEncoder<'_>) -> Result<(), ProtocolError> {
        ...
        self.algorithm.emit(&mut encoder)?;
        encoder.emit_u16(
            (self.time >> 32).try_into().map_err(|_| {
                ProtocolError::from("invalid time, overflow 48 bit counter in TSIG")
            })?,
        )?;
        ...
        encoder.emit_u16(
            self.mac
                .len()
                .try_into()
                .map_err(|_| ProtocolError::from("invalid mac, longer than 65535 B in TSIG"))?,
        )?;
        ...
        encoder.emit_u16(self.other.len().try_into().map_err(|_| {
            ProtocolError::from("invalid other_buffer, longer than 65535 B in TSIG")
        })?)?;
        encoder.emit_vec(&self.other)?;
        Ok(())
    }
...
pub fn signed_bitmessage_to_buf(
    ...
    let Some(tsig_rr) = sig else {
        return Err(ProtocolError::from("TSIG signature record not found"));
    };
