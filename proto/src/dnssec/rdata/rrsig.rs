...
use crate::{
    ...
    error::ProtocolError,
    rr::{
        DNSClass, RData, Record, RecordData, RecordDataDecodable, RecordSet, RecordType, TimeToLive,
    },
...
impl RRSIG {
    ...
    pub fn from_rrset(
        ...
        let tbs = TBS::from_input(
            ...
            zone_class,
            &input,
            rr_set.records(/* without rrsigs */ false),
        )?;
    ...
    pub fn authenticated_ttl(&self, record: &Record, current_time: u32) -> u32 {
        record
            .ttl
            .min(self.input.original_ttl.into())
            .min(TimeToLive::from(
                self.input.sig_expiration.0.saturating_sub(current_time),
            ))
    }
}
...
mod tests {
    ...
    fn test_get_filter() {
        ...
        assert!(rrset.records(true).any(|r| {
        ...
        assert!(rrset.records(/* with rrsigs */ true).any(|r| {
            if let RData::DNSSEC(DNSSECRData::RRSIG(sig)) = &r.rdata {
