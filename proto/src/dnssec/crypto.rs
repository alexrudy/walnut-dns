...
use crate::{error::ProtocolError, serialize::binary::DecodeError};
...
pub(super) fn decode_public_key<'a>(
    public_key: &'a [u8],
    algorithm: Algorithm,
) -> Result<Arc<dyn PublicKey + 'a>, ProtocolError> {
...
impl<'k> Ed25519<'k> {
    ...
    ///  of [RFC8032].
    /// ```
    pub fn from_public_bytes(public_key: Cow<'k, [u8]>) -> Result<Self, ProtocolError> {
...
impl PublicKey for Ed25519<'_> {
    ...
    fn verify(&self, message: &[u8], signature: &[u8]) -> Result<(), ProtocolError> {
        let public_key = signature::UnparsedPublicKey::new(&signature::ED25519, self.raw.as_ref());
        public_key
            .verify(message, signature)
            .map_err(|_| ProtocolError::Crypto("ED25519 signature verification failed"))
    }
...
impl<'k> Rsa<'k> {
    ...
    /// ```
    pub fn from_public_bytes(raw: &'k [u8], algorithm: Algorithm) -> Result<Self, ProtocolError> {
...
impl PublicKey for Rsa<'_> {
    ...
    fn verify(&self, message: &[u8], signature: &[u8]) -> Result<(), ProtocolError> {
        ...
        public_key
            ...
            .map_err(|_| ProtocolError::Crypto("RSA signature verification failed"))
    }
