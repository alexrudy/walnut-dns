...
impl DnssecSigner {
    ...
    pub fn sign(&self, tbs: &TBS) -> Result<Vec<u8>, ProtocolError> {
        self.key
            .sign(tbs)
            .map_err(|e| ProtocolError::Msg(format!("signing error: {e}")))
    }
    ...
    ///  }
    /// ```
    pub fn calculate_key_tag(&self) -> Result<u16, ProtocolError> {
...
mod tests {
    ...
    use crate::rr::{DNSClass, Name, RData, Record, RecordType, SerialNumber, TimeToLive};
