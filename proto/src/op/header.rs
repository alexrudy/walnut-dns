...
use crate::{
    ...
    serialize::binary::{
        decoder::{BinDecoder, DecodeError},
        encoder::{BinEncoder, EncodedSize},
        *,
    },
};
...
impl BinEncodable for Header {
    fn emit(&self, encoder: &mut BinEncoder<'_>) -> Result<(), ProtocolError> {
        ...
        encoder.emit_u8(r_z_ad_cd_rcod)?;
...
impl<'r> BinDecodable<'r> for Header {
    fn read(decoder: &mut BinDecoder<'_>) -> Result<Self, DecodeError> {
        ...
        let q_opcd_a_t_r = decoder.read_u8()?.unverified();
        ...
        let r_z_ad_cd_rcod = decoder.read_u8()?.unverified(); // fail fast...
